...
## Caveats
* If you load a save game for the second time in a session (first time works just fine) and get to a point where a voice line is supposed to play, the game crashes. I have NO fucking idea why this happens.
* The voices don't cut out when you advance to the next line. Not a big deal when reading normally, but I'd recommend not fastforwarding too much if you value your hearing. This is because the game lacks a dedicated voice function so I had to use normal sound effect calls.
...
* Expect a bunch of unvoiced or mismatched lines. The matching script I wrote did not take nametags into account to make it easier for me so if multiple characters share a line written the exact same, mistakes are bound to happen. Work in progress.
